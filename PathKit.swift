...
public struct Path {
  ...
  public static let separator = "/"
    public var parentDirectoryReferences: [String]?

  /// The underlying string representation
  internal var path: String {
      didSet {
        parentDirectoryReferences = nil
      }
  }
...
extension Path {
  ...
  public func children() throws -> [Path] {
    var array: [Path] = []
    let d = path.withCString { buffer in
        return opendir(buffer)
    }
    guard d != nil else { abort() }
    while let dir = readdir(d) {
        withUnsafePointer(to: &dir.pointee.d_name) { string in
            //string.with
            string.withMemoryRebound(to: Int8.self, capacity: 1000, { (innerString: UnsafePointer<Int8>) in
                let swiftString = String(cString: innerString)
                if swiftString != "." && swiftString != ".." {
                    array.append(self + Path(swiftString))
                }
            })
        }
        /*let pointer = UnsafeMutable
        let string = strdup(&dir.pointee.d_name)
        return self + Path($0)*/
    }
    closedir(d)
    return array
    /*let other = try Path.fileManager.contentsOfDirectory(atPath: path).map {
        self + Path($0)
    }
    precondition(other.count == array.count)
    zip(other, array).forEach { ps in
        precondition(ps.0.string == ps.1.string)
    }
    //assert(other == array)
    return other*/
  }
...
internal func +(lhs: String, rhs: String) -> Path {
  ...
  if rhs.hasPrefix(Path.separator) {
    ...
    /*if let short = short {
        assert(short == Path(components: lSlice + rSlice))
    }*/
